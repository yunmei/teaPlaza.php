...
class ApiBase
{
	...
	const DEFAULT_PAGENUM = 20; 								// 默认一页的数量
	...
	{
		return UserSession::getActiveSign();
	}
	
	public static function checkSid($format)
	{
		$sid = strip_tags(trim($_GET['sid']));
		$userSession = UserSession::checkSid($sid);
		if($userSession) {
			return $userSession->user_id;
		} else {
			$data = array('errorCode' => ApiError::SID_CHECK_INVALID, 'errorMessage'=>'SID验证失败');
			self::renderData($data, $format);
		}
		exit(0);
	}
